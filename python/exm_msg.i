...
%{
  ...
  struct PythonCallback {
    ...
    void operator()(const example::cb_id& id) {
      ...
      if (result == NULL) {
	PyErr_Print();
	INTERNAL("function call (%s, %d) associated with %s returned NULL", id.name.c_str(), id.priority, c_str_name);
        disable();
      }
    ...
    void operator()(const example::cb_id& id, unsigned int level, timespec& when, char* severity, const example::tag* tag, char *file, unsigned int line, char* text) {
      ...
      if (result == NULL) {
	PyErr_Print();
	ERROR("function call (%s, %d) associated with %s returned NULL", id.name.c_str(), id.priority, c_str_name);
